...
class RiveSwitch(context: Context, attrs: AttributeSet? = null) :
    ...
    init {
        context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.RiveSwitch,
            0, 0
        ).apply {
            try {
                val resourceId = getResourceId(
                    R.styleable.RiveSwitch_riveResource,
                    -1
                )
                stateMachineName =
                    getString(R.styleable.RiveSwitch_riveStateMachine);

                onAnimation = defaultedString(
                    getString(R.styleable.RiveSwitch_riveOnAnimation),
                    "on"
                )
                offAnimation = defaultedString(
                    getString(R.styleable.RiveSwitch_riveOffAnimation),
                    "off"
                )

                booleanStateInput = defaultedString(
                    getString(R.styleable.RiveSwitch_riveBooleanStateInput),
                    "toggle"
                )
